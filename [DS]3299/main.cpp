    ...
    for (j = 0; j < BM; ++j) {
      ...
      switch (rot) {
        case 0:
          ...
          break;  //   0
        case 1:
          B[j][BM - 1 - i] = A[sr + i][sc + j];
          break;  //  90
        case 2:
          B[BM - 1 - i][BM - 1 - j] = A[sr + i][sc + j];
          break;  // 180
        case 3:
          B[BM - 1 - j][i] = A[sr + i][sc + j];
          break;  // 270
      }
    }
  if (mr < 0) return 1;
...
static int run(int usedCount) {
  ...
  if (userR != ans[2] || userC != ans[3]) {
    userR = userAns[0];
    userC = userAns[1];
    if (userR < 0 || userR > limit || userC < 0 || userC > limit) return 0;
    for (int i = 0; i < BM; ++i)
      for (int j = 0; j < BM; ++j) {
        int r = i, c = j;
        switch (userRot) {
        ...
        if (A[userR + i][userC + j] != !!B[r][c]) return 0;
      }
    sr = userR, sc = userC;
  }
